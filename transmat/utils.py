...
def score(sp1, sp2, gold, additional):
    ...
    for i,el1 in enumerate(gold.keys()):
        ...
        print ("\nId: %d Source: %s \n\tTranslation:\n%s \n\tGold: %s \n\tRank: %d" %(len(ranks), el1, translations, gold[el1], rnk))
        
	sp2_idx = srtd_idx[rnk-1, sp1_idx]
        word, score = sp2.id2row[sp2_idx], -sim_mat[sp2_idx, sp1_idx]


	#uncomment for printing result
        #print ("\nId: %d Source: %s \n\tTranslation:\n%s \n\tGold: %s \n\tRank: %d \n\tAnswers Score: %f" %
         #      (len(ranks), el1, translations, gold[el1], rnk,score))
	print el1,word,score
